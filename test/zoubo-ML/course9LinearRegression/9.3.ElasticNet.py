...
if __name__ == "__main__":
    warnings.filterwarnings(action='ignore', category=ConvergenceWarning)  #忽略警告
    np.random.seed(0)
    np.set_printoptions(linewidth=300)  #使得输出结果更加好看
    N = 9
    x = np.linspace(0, 6, N) + np.random.randn(N)  #在0-6之间生成9个数字
    x = np.sort(x)   #对数字进行排序
    y = x**2 - 4*x - 3 + np.random.randn(N)
    x.shape = -1, 1  #数组变形,行变列
    y.shape = -1, 1
